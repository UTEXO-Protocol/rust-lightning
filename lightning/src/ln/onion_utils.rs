...
trait PathHop {
	...
	fn payment_amount(&self) -> Option<u64>;
}
...
pub enum LocalHTLCFailureReason {
	...
	/// The HTLC was failed because the forarding node didn't send what we were expecting.
	RgbSentUnexpected,
	/// The HTLC's RGB amount is less than the amount specified by the sender.
	///
	/// The forwarding node has tampered with this value, or has a bug in its implementation.
	RgbFinalIncorrectHTLCAmount,
	/// The HTLC was failed because its RGB amount is more than then largest HTLC that the channel
	/// can currently accept.
	///
	/// This may occur because the outbound channel has insufficient liquidity to forward the HTLC,
	/// we have reached the counterparty's in-flight limits, or the HTLC exceeds our advertised
...
impl LocalHTLCFailureReason {
	pub(super) fn failure_code(&self) -> u16 {
		match self {
			...
			| Self::RgbSentUnexpected
			| Self::RgbFinalIncorrectHTLCAmount => 19,
...
ser_failure_reasons!(
	...
	(43, RgbSentUnexpected),
	(44, RgbFinalIncorrectHTLCAmount),
	(45, RgbHTLCMaximum)
);
...
impl HTLCFailReason {
	pub(super) fn reason(failure_reason: LocalHTLCFailureReason, data: Vec<u8>) -> Self {
		match failure_reason {
			...
			| LocalHTLCFailureReason::RgbSentUnexpected
			| LocalHTLCFailureReason::RgbFinalIncorrectHTLCAmount => debug_assert_eq!(data.len(), 8),
