...
pub enum Event {
	...
	HTLCIntercepted {
		...
		is_swap: bool,
		/// The previous short channel id alias
		...
		match self {
			...
			&Event::HTLCIntercepted {
				...
				write_tlv_fields!(writer, {
					...
					(9, inbound_rgb_amount, option),
					(11, expected_outbound_rgb_payment, option),
					(12, is_swap, required),
					(14, prev_outbound_scid_alias, required),
				});
			},
	...
	fn read<R: io::Read>(reader: &mut R) -> Result<Option<Self>, msgs::DecodeError> {
		match Readable::read(reader)? {
			...
			6u8 => {
				...
				let mut prev_outbound_scid_alias = 0;
				read_tlv_fields!(reader, {
					(0, intercept_id, required),
